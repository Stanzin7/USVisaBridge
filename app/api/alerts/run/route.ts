...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const report of reports) {
      ...
      if (prefError) {
        console.error(
          `[ALERTS] Error fetching preferences for report ${report.id}:`,
          prefError
        );
        continue;
      }
      ...
      const matchingPrefs = preferences.filter((pref) => {
        if (
          pref.date_start &&
          new Date(report.earliest_date) < new Date(pref.date_start)
        ) {
          return false;
        }
        if (
          pref.date_end &&
          new Date(report.earliest_date) > new Date(pref.date_end)
        ) {
          return false;
        }
        return true;
      ...
      for (const pref of matchingPrefs) {
        ...
        for (const channel of pref.channels) {
          ...
          if (alertInsertError) {
            console.error(
              `[ALERTS] Error creating alert record:`,
              alertInsertError
            );
            continue;
          }
