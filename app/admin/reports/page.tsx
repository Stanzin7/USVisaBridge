...
export default function AdminReportsPage() {
  ...
  const handleDecision = async (reportId: string, decision: 'verified' | 'rejected') => {
    try {
        ...
        <div className="space-y-4">
          {reports.map((report) => (
            <div key={report.id} className="bg-white p-6 rounded-lg shadow">
              ...
              <div className="mb-4">
                <p className="text-sm">
                  <strong>Earliest Date:</strong> {new Date(report.earliest_date).toLocaleDateString()}
                </p>
                {report.latest_date && (
                  <p className="text-sm">
                    <strong>Latest Date:</strong> {new Date(report.latest_date).toLocaleDateString()}
                  </p>
                )}
              </div>
              ...
              {report.screenshot_path && (
                <div className="mb-4">
                  ...
                  <img
                    src={`/api/screenshots/${encodeURIComponent(report.screenshot_path)}`}
